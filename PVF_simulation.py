    ...
    for discount in DISCOUNT:
        for dimension in DIMENSION:
            for grid_size in range(5,11):
                ...
                plot_results(n2v_all_results, pvf_all_results, grid_size, reward_location, dimension, discount, num_samples)
...
def plot_results( pvf_all_results, grid_size, reward_location, dimension, discount, num_samples):
    ...
    ax = axs[0, 0]
    ax.errorbar(sum([range(reward_location), range(grid_size,grid_size*grid_size)],[]),n2v_mean_steps_to_goal, yerr=n2v_std_steps_to_goal, fmt='bo',ecolor='blue')
    ...
    ax = axs[1, 0]
    ax.errorbar(sum([range(reward_location), range(grid_size, grid_size * grid_size)], []), n2v_mean_cumulative_rewards,
    ...
    #plt.savefig('plots_nonneg/'+str(grid_size) + 'grid_' + str(dimension) + 'dimension_' + str(discount) + 'discount_' + str(
