...
class DiscountedNode2Vec:
    ...
    def __init__(self, num_states, dimension, window_size, walks, discount):
        self.N = num_states
    ...
    def get_neighborhood(self, walk):
        ...
        for current_node_index in range(len(walk)):
            ...
            for w in range(-self.window_size, self.window_size + 1):
                if 0 <= current_node_index + w < len(walk) and w != 0:
                    ...
                    output.append(self.encode_one_hot(walk[current_node_index + w], self.gamma ** abs(w)))
    ...
    def process_walks(self):
        ...
        for walk in self.walks:
            input, output, vocab = self.get_neighborhood(walk)
    ...
    def train_discounted_n2v(self, batch_size=32, learning_rate=0.01, num_epochs=2):
        ...
        output_node = tf.placeholder(tf.int32, shape=[None,self.N])
        ...
        weights = tf.Variable(tf.random_normal([self.N, self.dimension]))
